...
class Pi05PipelineSm89Fp8Ffn:
    ...
    def _vision_layer(self, i: int, stream: int):
        """One SigLIP layer - FP16 (same as original)."""
        fvk = self.fvk
        gemm = self.gemm
        W = self.weights
        B = self.bufs
    ...
    def _apply_rope_kernel(self, x_flat_ptr, cos_ptr, sin_ptr, seq, num_heads, head_dim, stream):
        """Apply RoPE using optimized kernel instead of torch ops."""
