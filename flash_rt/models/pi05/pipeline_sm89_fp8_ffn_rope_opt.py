"""FlashRT — SM89 Pi0.5 inference pipeline with FP8 FFN + optimized RoPE kernel.

Key optimizations:
1. encoder_seq_len padding: 265 -> 272 (FP8 kernel alignment)
2. Encoder FFN uses FP8 GEMM: 1.57x speedup
3. RoPE uses optimized kernel instead of torch ops: 8x speedup
4. Vision, Encoder Attention, Decoder use FP16

Performance improvement: ~27.75ms saved from RoPE optimization
"""
...
class Pi05PipelineSm89Fp8FfnRopeOpt:
    """Pi0.5 pipeline with FP8 FFN + optimized RoPE kernel."""
    ...
    def __init__(
        ...
        *,
        ...
        self.use_fp8_ffn = bool(weights_fp8)
        ...
        logger.info("Pi05PipelineSm89Fp8FfnRopeOpt initialized (RoPE kernel optimized)")
    ...
    def _create_preallocated_tensors(self):
        ...
        # RoPE专用tensors (flat layout for kernel)
        self._dec_Q_flat = torch.empty(sd, DEC_NH * DEC_HD, dtype=torch.float16, device='cuda')
        self._dec_K_flat = torch.empty(sd, DEC_NKV * DEC_HD, dtype=torch.float16, device='cuda')
        ...
        # Encoder RoPE专用flat tensors
        self._enc_Q_flat = torch.empty(es, ENC_NH * ENC_HD, dtype=torch.float16, device='cuda')
        self._enc_K_flat = torch.empty(es, ENC_NKV * ENC_HD, dtype=torch.float16, device='cuda')
    ...
    def _ada_rms_norm_fp16(self, x_ptr, style_ptr, out_ptr, gate_out_ptr, seq, dim, stream):
        self.fvk.adarms_fp16(x_ptr, style_ptr, out_ptr, gate_out_ptr, seq, dim, stream)
    
    def _gate_residual_fp16(self, x_ptr, residual_ptr, gate_ptr, seq_dim, stream):
    ...
    def _apply_rope_kernel_optimized(self, x_flat_ptr, cos_ptr, sin_ptr, seq, num_heads, head_dim, stream):
        """Apply RoPE using optimized kernel - 8x faster than torch ops."""
        self.fvk.rope_rotate_half_fp16(x_flat_ptr, cos_ptr, sin_ptr, seq, num_heads, head_dim, stream)
    ...
    def vision_encoder(self, stream: int = 0):
        ...
        fvk.patch_im2col(_p(B["observation_images_normalized"]), _p(B["vision_patches"]), nv, stream)
        gemm.fp16_nn(_p(B["vision_patches"]), W["vision_patch_embedding_w"], _p(B["vision_x"]), seq, VIS_D, VIS_PATCH_FLAT, stream=stream)
        fvk.bias_residual_fp16(_p(B["vision_x"]), _p(B["vision_pos_embed_expanded"]), W["vision_patch_embedding_b"], seq, VIS_D, stream=stream)
    ...
    def _vision_layer(self, i: int, stream: int):
        ...
        fvk.layer_norm_fp16(_p(B["vision_x"]), W["vision_pre_attn_norm_w"][i], W["vision_pre_attn_norm_b"][i], _p(B["vision_x_norm"]), seq, VIS_D, 1e-5, stream=stream)
        gemm.fp16_nn(_p(B["vision_x_norm"]), W["vision_attn_qkv_w"][i], _p(B["vision_QKV"]), seq, 3 * VIS_D, VIS_D, stream=stream)
        ...
        gemm.fp16_nn(_p(B["vision_attn_out"]), W["vision_attn_o_w"][i], _p(B["vision_x_norm"]), seq, VIS_D, VIS_D, stream=stream)
        fvk.bias_residual_fp16(_p(B["vision_x"]), _p(B["vision_x_norm"]), W["vision_attn_o_b"][i], seq, VIS_D, stream=stream)
        
        fvk.layer_norm_fp16(_p(B["vision_x"]), W["vision_pre_ffn_norm_w"][i], W["vision_pre_ffn_norm_b"][i], _p(B["vision_x_norm"]), seq, VIS_D, 1e-5, stream=stream)
        gemm.fp16_nn(_p(B["vision_x_norm"]), W["vision_ffn_up_w"][i], _p(B["vision_hidden"]), seq, VIS_H, VIS_D, stream=stream)
        ...
        gemm.fp16_nn(_p(B["vision_hidden"]), W["vision_ffn_down_w"][i], _p(B["vision_ffn_out"]), seq, VIS_D, VIS_H, stream=stream)
        fvk.add_bias_fp16(_p(B["vision_ffn_out"]), W["vision_ffn_down_b"][i], seq, VIS_D, stream=stream)
        fvk.residual_add_fp16(_p(B["vision_x"]), _p(B["vision_ffn_out"]), seq * VIS_D, stream)
    
    # ══════════════════════════════════════════════════════════════════
    #  Encoder - Attention FP16, FFN FP8, RoPE optimized
    # ══════════════════════════════════════════════════════════════════
    ...
    def transformer_encoder(self, stream: int = 0):
        ...
        fvk.layer_norm_fp16(_p(B["vision_x"]), W["vision_final_norm_w"], W["vision_final_norm_b"], _p(B["vision_x_norm"]), vs, VIS_D, 1e-5, stream=stream)
        gemm.fp16_nn(_p(B["vision_x_norm"]), W["encoder_multi_modal_projector_w"], _p(B["encoder_x"]), vs, ENC_D, VIS_D, stream=stream)
        fvk.add_bias_fp16(_p(B["encoder_x"]), W["encoder_multi_modal_projector_b"], vs, ENC_D, stream=stream)
        
        for i in range(self.enc_layers):
            self._encoder_layer_opt(i, es, stream)
    
    def _encoder_layer_opt(self, i: int, seq: int, stream: int):
        """Encoder layer with optimized RoPE kernel."""
        fvk = self.fvk
        gemm = self.gemm
        W = self.weights
        W8 = self.weights_fp8
        ...
        fvk.rms_norm_fp16(_p(B["encoder_x"]), _p(B["encoder_ones"]), _p(B["encoder_x_norm"]), seq, ENC_D, 1e-6, stream=stream)
        
        gemm.fp16_nn(_p(B["encoder_x_norm"]), W["encoder_attn_qkv_w"][i], _p(B["encoder_QKV"]), seq, (ENC_NH + 2 * ENC_NKV) * ENC_HD, ENC_D, stream=stream)
        
        enc_qkv_dim = (ENC_NH + 2 * ENC_NKV) * ENC_HD
        fvk.gpu_copy(self._enc_qkv_tensor.data_ptr(), _p(B["encoder_QKV"]), seq * enc_qkv_dim * 2, stream)
        
        # Split QKV - 使用flat tensors
        Q_flat = self._enc_qkv_tensor[:, :ENC_NH * ENC_HD]  # [S, NH*HD]
        K_flat = self._enc_qkv_tensor[:, ENC_NH * ENC_HD:ENC_NH * ENC_HD + ENC_NKV * ENC_HD]  # [S, NKV*HD]
        V = self._enc_qkv_tensor[:, ENC_NH * ENC_HD + ENC_NKV * ENC_HD:]  # [S, NKV*HD]
        
        # 加载RoPE table
        fvk.gpu_copy(self._enc_rope_tensor.data_ptr(), _p(B["encoder_rope"]), seq * 256 * 2, stream)
        
        # 使用优化kernel应用RoPE
        self._apply_rope_kernel_optimized(Q_flat.data_ptr(), self._enc_rope_tensor.data_ptr(), 
                                          self._enc_rope_tensor.data_ptr(), seq, ENC_NH, ENC_HD, stream)
        self._apply_rope_kernel_optimized(K_flat.data_ptr(), self._enc_rope_tensor.data_ptr(),
                                          self._enc_rope_tensor.data_ptr(), seq, ENC_NKV, ENC_HD, stream)
        
        # 转回3D格式用于attention
        Q_rope = Q_flat.view(seq, ENC_NH, ENC_HD)
        K_rope = K_flat.view(seq, ENC_NKV, ENC_HD)
        ...
        gemm.fp16_nn(_p(B["encoder_attn_out"]), W["encoder_attn_o_w"][i], _p(B["encoder_x_norm"]), seq, ENC_D, ENC_D, stream=stream)
    ...
    def _encoder_ffn_fp16(self, i: int, seq: int, stream: int):
        ...
        gemm.fp16_nn(_p(B["encoder_x_norm"]), W["encoder_ffn_gate_w"][i], _p(B["encoder_gate_merged"]), seq, ENC_H, ENC_D, stream=stream)
        gemm.fp16_nn(_p(B["encoder_x_norm"]), W["encoder_ffn_up_w"][i], _p(B["encoder_hidden"]), seq, ENC_H, ENC_D, stream=stream)
        fvk.gate_geglu_fp16(_p(B["encoder_gate_merged"]), _p(B["encoder_hidden"]), _p(B["encoder_hidden"]), seq * ENC_H, stream=stream)
        gemm.fp16_nn(_p(B["encoder_hidden"]), W["encoder_ffn_down_w"][i], _p(B["encoder_ffn_out"]), seq, ENC_D, ENC_H, stream=stream)
    ...
    def _encoder_ffn_fp8(self, i: int, seq: int, stream: int):
        ...
        fvk.quantize_fp8_static_fp16(_p(B["encoder_x_norm"]), self.encoder_x_fp8.data_ptr(), self.scale_one_ptr, seq * ENC_D, stream)
        
        frk.cutlass_ada_fp8_gemm_bf16_simple(self.encoder_x_fp8.data_ptr(), W8["gate"][i], _p(B["encoder_gate_merged"]), seq, ENC_H, ENC_D, 1.0, S8["gate"][i], stream)
        frk.cutlass_ada_fp8_gemm_bf16_simple(self.encoder_x_fp8.data_ptr(), W8["up"][i], _p(B["encoder_hidden"]), seq, ENC_H, ENC_D, 1.0, S8["up"][i], stream)
        
        fvk.gate_geglu_fp16(_p(B["encoder_gate_merged"]), _p(B["encoder_hidden"]), _p(B["encoder_hidden"]), seq * ENC_H, stream=stream)
        
        fvk.quantize_fp8_static_fp16(_p(B["encoder_hidden"]), self.encoder_hidden_fp8.data_ptr(), self.scale_one_ptr, seq * ENC_H, stream)
        
        frk.cutlass_ada_fp8_gemm_bf16_simple(self.encoder_hidden_fp8.data_ptr(), W8["down"][i], _p(B["encoder_ffn_out"]), seq, ENC_D, ENC_H, 1.0, S8["down"][i], stream)
        
        fvk.residual_add_fp16(_p(B["encoder_x"]), _p(B["encoder_ffn_out"]), seq * ENC_D, stream)
    
    # ══════════════════════════════════════════════════════════════════
    #  Decoder + Diffusion - FP16, optimized RoPE
    # ══════════════════════════════════════════════════════════════════
    ...
    def _assemble_decoder_x(self, step: int, stream: int):
        ...
        self._cudart.cudaMemcpyAsync(ctypes.c_void_p(_p(B["decoder_x"])), ctypes.c_void_p(time_emb_ptr), DEC_D * 2, 3, stream)
        
        x_action_ptr = _p(B["decoder_x"]) + DEC_D * 2
        
        gemm.fp16_nn(_p(B["diffusion_noise"]), W["decoder_action_in_proj_w"], x_action_ptr, sa, DEC_D, ACTION_DIM, stream=stream)
        fvk.add_bias_fp16(x_action_ptr, W["decoder_action_in_proj_b"], sa, DEC_D, stream=stream)
    ...
    def transformer_decoder(self, stream: int = 0):
        ...
        for step in range(self.num_steps):
            ...
            for i in range(self.dec_layers):
                self._decoder_layer_opt(i, step, es, sd, stream)
            
            style_final_ptr = self._style_slice_ptr("decoder_style_final", step)
            
            self._ada_rms_norm_fp16(_p(B["decoder_x"]), style_final_ptr, _p(B["x_normed_buf"]), _p(B["gate_buf"]), sd, DEC_D, stream)
            
            x_out_action_ptr = _p(B["x_normed_buf"]) + DEC_D * 2
            
            gemm.fp16_nn(x_out_action_ptr, W["decoder_action_out_proj_w"], _p(B["diffusion_noise"]), sd - 1, ACTION_DIM, DEC_D, stream=stream)
            self.fvk.add_bias_fp16(_p(B["diffusion_noise"]), W["decoder_action_out_proj_b"], sd - 1, ACTION_DIM, stream)
    
    def _decoder_layer_opt(self, i: int, step: int, enc_seq: int, sd: int, stream: int):
        """Decoder layer with optimized RoPE kernel."""
        fvk = self.fvk
        gemm = self.gemm
        W = self.weights
        B = self.bufs
        ...
        self._ada_rms_norm_fp16(_p(B["decoder_x"]), style_attn_ptr, _p(B["x_normed_buf"]), _p(B["gate_buf"]), sd, DEC_D, stream)
        
        dec_qkv_dim = (DEC_NH + 2 * DEC_NKV) * DEC_HD
        gemm.fp16_nn(_p(B["x_normed_buf"]), W["decoder_attn_qkv_w"][i], _p(B["decoder_QKV"]), sd, dec_qkv_dim, DEC_D, stream=stream)
        
        # Split QKV using pre-allocated flat tensors
        ...
        V_flat = qkv_tensor[:, DEC_NH * DEC_HD + DEC_NKV * DEC_HD:]  # [S, NKV*HD]
        
        # 加载RoPE table
        fvk.gpu_copy(self._dec_rope_tensor.data_ptr(), _p(B["decoder_rope"]), sd * 256 * 2, stream)
        
        # 使用优化kernel应用RoPE
        self._apply_rope_kernel_optimized(Q_flat.data_ptr(), self._dec_rope_tensor.data_ptr(),
                                          self._dec_rope_tensor.data_ptr(), sd, DEC_NH, DEC_HD, stream)
        self._apply_rope_kernel_optimized(K_flat.data_ptr(), self._dec_rope_tensor.data_ptr(),
                                          self._dec_rope_tensor.data_ptr(), sd, DEC_NKV, DEC_HD, stream)
        
        # 转回3D格式
        Q_rope = Q_flat.view(sd, DEC_NH, DEC_HD)
        K_rope = K_flat.view(sd, DEC_NKV, DEC_HD)
        V = V_flat.view(sd, DEC_NKV, DEC_HD)
        ...
        gemm.fp16_nn(_p(B["decoder_attn_out"]), W["decoder_attn_o_w"][i], _p(B["x_normed_buf"]), sd, DEC_D, DEC_NH * DEC_HD, stream=stream)
        
        self._gate_residual_fp16(_p(B["decoder_x"]), _p(B["x_normed_buf"]), _p(B["gate_buf"]), sd * DEC_D, stream)
        
        # AdaRMSNorm (FFN)
        self._ada_rms_norm_fp16(_p(B["decoder_x"]), style_ffn_ptr, _p(B["x_normed_buf"]), _p(B["gate_buf"]), sd, DEC_D, stream)
        
        gemm.fp16_nn(_p(B["x_normed_buf"]), W["decoder_ffn_gate_w"][i], _p(B["decoder_gate_merged"]), sd, DEC_H, DEC_D, stream=stream)
        gemm.fp16_nn(_p(B["x_normed_buf"]), W["decoder_ffn_up_w"][i], _p(B["decoder_hidden"]), sd, DEC_H, DEC_D, stream=stream)
        
        fvk.gate_geglu_fp16(_p(B["decoder_gate_merged"]), _p(B["decoder_hidden"]), _p(B["decoder_hidden"]), sd * DEC_H, stream=stream)
        
        gemm.fp16_nn(_p(B["decoder_hidden"]), W["decoder_ffn_down_w"][i], _p(B["decoder_attn_out"]), sd, DEC_D, DEC_H, stream=stream)
        
        self._gate_residual_fp16(_p(B["decoder_x"]), _p(B["decoder_attn_out"]), _p(B["gate_buf"]), sd * DEC_D, stream)
    ...
    def _copy_lang_embeds_to_encoder_x(self, stream: int):
        ...
        self._cudart.cudaMemcpyAsync(ctypes.c_void_p(dst_ptr), self._lang_embeds_buf.ptr, self._lang_embeds_buf.nbytes, 3, stream)
