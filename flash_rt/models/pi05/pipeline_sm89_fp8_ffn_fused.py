...
class Pi05PipelineSm89Fp8Ffn:
    ...
    def _decoder_layer(self, i: int, step: int, enc_seq: int, sd: int, stream: int):
        ...
        # Session 143优化: Gate+Up融合GEMM
        # 原本2个独立GEMM: x_norm @ gate_w 和 x_norm @ up_w
        # 现在合并为: x_norm @ gate_up_w (输出2*DEC_H维度)
        # 使用gate_geglu_merged_fp16直接处理merged buffer
        
        # 检查是否有融合权重
        if "decoder_ffn_gate_up_w" in W and i in W.get("decoder_ffn_gate_up_w", {}):
            # 使用融合的Gate+Up权重 (输出2*DEC_H维度)
            gemm.fp16_nn(
                _p(B["x_normed_buf"]),
                W["decoder_ffn_gate_up_w"][i],
                _p(B["decoder_gate_merged"]),
                sd, 2 * DEC_H, DEC_D, stream=stream)
            
            # 使用merged GEGLU kernel直接处理合并的gate+up
            # merged buffer: [sd, 2*DEC_H], 前半是gate，后半是up
            # 输出到decoder_hidden: [sd, DEC_H]
            fvk.gate_geglu_merged_fp16(
                _p(B["decoder_gate_merged"]),  # merged buffer
                _p(B["decoder_hidden"]),        # output
                sd,                             # seq length
                DEC_H,                          # half_dim (单边维度)
                stream)
        else:
            # Fallback: 使用原始分离的Gate和Up
            gemm.fp16_nn(
                _p(B["x_normed_buf"]),
