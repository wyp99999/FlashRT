"""FlashRT -- SM89 FP8 Vision+Encoder FFN Pi0.5 torch frontend.

Optimized version with FP8 for both Vision FFN and Encoder FFN.

Key optimizations:
- Vision FFN: FP8 GEMM (seq=512, ~18x speedup)
- Encoder FFN: FP8 GEMM (seq=528, ~1.5x speedup)
...
from flash_rt.models.pi05.pipeline_sm89_fp8_vision_encoder import (
    Pi05PipelineSm89Fp8VisionEncoder,
...
class Pi05TorchFrontendSm89Fp8Full:
    """SM89 FP8 Vision+Encoder FFN Pi0.5 frontend.
    
    FP8 optimization for both Vision FFN and Encoder FFN.
    """
    ...
    def __init__(self, checkpoint_dir, num_views=2, chunk_size=CHUNK_SIZE_DEFAULT,
        ...
        logger.info("Pi05TorchFrontendSm89Fp8Full initialised (num_views=%d, num_steps=%d, fp8=%s)",
                    self.num_views, self.num_steps, self.use_fp8)
    
    def _load_norm_stats(self, checkpoint_dir):
        from flash_rt.core.utils.norm_stats import load_norm_stats, lerobot_candidates
        candidates = [checkpoint_dir / "norm_stats.json", *lerobot_candidates(checkpoint_dir)]
        try:
    ...
    def _load_and_build_weights(self, safetensors_path):
        """Load weights with FP8 quantization for Vision and Encoder FFN."""
        from safetensors import safe_open
        from flash_rt.executors.torch_weights import _autodetect_strip_prefix
        
        logger.info("Loading weights with FP8 optimization (Vision + Encoder FFN)")
        ...
        vis_up_w_fp16, vis_up_b, vis_down_w_fp16, vis_down_b = [], [], [], []
        vis_up_w_fp8, vis_down_w_fp8 = [], []
        vis_up_scales, vis_down_scales = [], []
        ...
        for i in range(VIS_L):
            ...
            # Vision FFN - FP16 or FP8
            up_w = get(f"{lp}.mlp.fc1.weight").t()
            down_w = get(f"{lp}.mlp.fc2.weight").t()
            
            if self.use_fp8:
                # FP8 quantization for Vision FFN
                up_fp8, up_scale = _quantize_weight_fp8_colmajor(up_w)
                down_fp8, down_scale = _quantize_weight_fp8_colmajor(down_w)
                vis_up_w_fp8.append(upload(up_fp8))
                vis_down_w_fp8.append(upload(down_fp8))
                vis_up_scales.append(up_scale)
                vis_down_scales.append(down_scale)
                # Placeholder for FP16 (not used)
                vis_up_w_fp16.append(torch.empty(1, dtype=fp16, device='cuda'))
                vis_down_w_fp16.append(torch.empty(1, dtype=fp16, device='cuda'))
            else:
                vis_up_w_fp16.append(upload(up_w))
                vis_down_w_fp16.append(upload(down_w))
        ...
        if not self.use_fp8:
            self._ckpt_fp16["vision_ffn_up_w"] = upload(torch.stack(vis_up_w_fp16))
            self._ckpt_fp16["vision_ffn_down_w"] = upload(torch.stack(vis_down_w_fp16))
        else:
            self._ckpt_fp16["vision_ffn_up_w"] = upload(torch.empty(1, dtype=fp16, device='cuda'))
            self._ckpt_fp16["vision_ffn_down_w"] = upload(torch.empty(1, dtype=fp16, device='cuda'))
        ...
        # FP8 Vision weights
        if self.use_fp8:
            self._ckpt_fp8["vision_ffn_up"] = upload(torch.stack(vis_up_w_fp8))
            self._ckpt_fp8["vision_ffn_down"] = upload(torch.stack(vis_down_w_fp8))
            self._fp8_scales["vision_ffn_up"] = {i: s for i, s in enumerate(vis_up_scales)}
            self._fp8_scales["vision_ffn_down"] = {i: s for i, s in enumerate(vis_down_scales)}
        ...
        for i in range(ENC_L):
            ...
            if self.use_fp8:
                ...
                down_w = get(f"{ep}.{i}.mlp.down_proj.weight")
                
                gate_fp8, gate_scale = _quantize_weight_fp8_colmajor(gate_w.t().to(fp16))
                up_fp8, up_scale = _quantize_weight_fp8_colmajor(up_w.t().to(fp16))
                down_fp8, down_scale = _quantize_weight_fp8_colmajor(down_w)
        ...
        logger.info("Built weights, GPU: %.1f MB", torch.cuda.memory_allocated() / 1024**2)
    ...
    def build_pipeline(self):
        ...
        fp8_scales = self._fp8_scales if self.use_fp8 else {}
        
        max_prompt_len = getattr(self, '_prompt_len', self.max_prompt_len)
        
        self._pipeline = Pi05PipelineSm89Fp8VisionEncoder(
    ...
    def _build_pipeline_weights(self):
        ...
        def p_list(k):
            t = W[k]
            if t.shape[0] == 1:  # Placeholder
                return [0] * (VIS_L if "vision" in k else ENC_L)
    ...
    def _build_fp8_weights(self):
        ...
        return {
            "vision_ffn_up": p_dict("vision_ffn_up"),
            "vision_ffn_down": p_dict("vision_ffn_down"),
            "gate": p_dict("gate"),
            "up": p_dict("up"),
            "down": p_dict("down"),
        }
