"""FlashRT -- SM89 FP8 Pi0.5 torch frontend (V3修复版).

基于Session 135修复的FP8 kernel调用方式:
- 权重预量化到FP8 ColumnMajor格式 (B.t().contiguous())
- 使用cutlass_ada_fp8_gemm_bf16 kernel
- scale参数使用device pointer
...
from flash_rt.models.pi05.pipeline_sm89_fp8_v3 import (
    Pi05PipelineSm89Fp8V3,
    FP8GemmRunnerV3,
...
def _quantize_weight_fp8_colmajor(w_fp16: torch.Tensor) -> tuple:
    ...
    Returns:
        (w_fp8_colmajor, scale_tensor): 
            - w_fp8_colmajor: FP8 tensor in ColumnMajor layout (N, K)
            - scale_tensor: FP32 scale tensor on device
    """
    # Compute scale (FP8 E4M3 max = 448)
    ...
    # Convert to ColumnMajor layout: B.t().contiguous()
    # This gives us (N, K) memory layout which equals ColumnMajor B (K, N)
    w_fp8_colmajor = w_fp8.t().contiguous()
    
    # Scale tensor on device
    scale_tensor = torch.tensor([scale], dtype=torch.float32, device='cuda')
    
    return w_fp8_colmajor, scale_tensor
...
class Pi05TorchFrontendSm89Fp8V3:
    """SM89 FP8优化Pi0.5 frontend (V3修复版).
    ...
    def __init__(self, checkpoint_dir, num_views=2, chunk_size=CHUNK_SIZE_DEFAULT,
        ...
        self._scale_store = []  # Store scale tensors
        self._ckpt_fp8 = {}
        self._ckpt_fp16 = {}
        self._fp8_scales = {}  # Per-weight scale values
        ...
        logger.info("Pi05TorchFrontendSm89Fp8V3 initialised (num_views=%d, num_steps=%d)",
    ...
    def _load_and_build_weights(self, safetensors_path):
        ...
        def upload_fp8_colmajor(w_fp16):
            """Upload FP8 weight in ColumnMajor format."""
            w_fp8_col, scale_tensor = _quantize_weight_fp8_colmajor(w_fp16)
            w_gpu = w_fp8_col.cuda().contiguous()
            self._weight_store_fp8.append(w_gpu)
            self._scale_store.append(scale_tensor)
            return w_gpu, scale_tensor
        ...
        # Vision layers - Keep FP16 (not dominant in performance)
        ...
        self._ckpt_fp16["vision_final_norm_w"] = upload_fp16(
            get(f"{vp}.post_layernorm.weight").to(fp16))
        self._ckpt_fp16["vision_final_norm_b"] = upload_fp16(
            get(f"{vp}.post_layernorm.bias").to(fp16))
        
        # Projector (FP16)
        mp = "paligemma_with_expert.paligemma.model.multi_modal_projector.linear"
        self._ckpt_fp16["encoder_multi_modal_projector_w"] = upload_fp16(
            get(f"{mp}.weight").t().to(fp16))
        self._ckpt_fp16["encoder_multi_modal_projector_b"] = upload_fp16(
            get(f"{mp}.bias").to(fp16))
        
        gc.collect()
        torch.cuda.empty_cache()
        
        # Encoder (Gemma-2B) - FP8 for large matrices
        ep = "paligemma_with_expert.paligemma.model.language_model.layers"
        enc_qkv_w_fp8, enc_qkv_scales = [], []
        enc_o_w_fp16 = []
        enc_gate_w_fp8, enc_gate_scales = [], []
        enc_up_w_fp8, enc_up_scales = [], []
        enc_down_w_fp8, enc_down_scales = [], []
        ...
        for i in range(ENC_L):
            ...
            w_fp8, scale = upload_fp8_colmajor(qkv_w)
            enc_qkv_w_fp8.append(w_fp8)
            enc_qkv_scales.append(scale)
            
            # O - FP16 (smaller matrix)
        ...
        self._fp8_scales["encoder_attn_qkv_w"] = torch.stack(enc_qkv_scales)
        self._ckpt_fp16["encoder_attn_o_w"] = torch.stack(enc_o_w_fp16)
        self._ckpt_fp8["encoder_ffn_gate_w"] = torch.stack(enc_gate_w_fp8)
        self._fp8_scales["encoder_ffn_gate_w"] = torch.stack(enc_gate_scales)
        self._ckpt_fp8["encoder_ffn_up_w"] = torch.stack(enc_up_w_fp8)
        self._fp8_scales["encoder_ffn_up_w"] = torch.stack(enc_up_scales)
        self._ckpt_fp8["encoder_ffn_down_w"] = torch.stack(enc_down_w_fp8)
        self._fp8_scales["encoder_ffn_down_w"] = torch.stack(enc_down_scales)
        
        gc.collect()
        torch.cuda.empty_cache()
        
        # Decoder (Gemma-300M) - FP16 (small matrices)
        ...
        for i in range(DEC_L):
            dec_attn_mod_w.append(upload_fp16(
                get(f"{dp}.{i}.input_layernorm.dense.weight").t().to(fp16)))
            dec_attn_mod_b.append(upload_fp16(
                get(f"{dp}.{i}.input_layernorm.dense.bias").to(fp16)))
            ...
            dec_ffn_mod_w.append(upload_fp16(
                get(f"{dp}.{i}.post_attention_layernorm.dense.weight").t().to(fp16)))
            dec_ffn_mod_b.append(upload_fp16(
                get(f"{dp}.{i}.post_attention_layernorm.dense.bias").to(fp16)))
        ...
        self._ckpt_fp16["decoder_action_out_proj_w"] = upload_fp16(
            (get("action_out_proj.weight").t().to(fp16) * dt_scale))
        self._ckpt_fp16["decoder_action_out_proj_b"] = upload_fp16(
            get("action_out_proj.bias").to(fp16) * dt_scale)
    ...
    def build_pipeline(self):
        ...
        weights_fp8, weights_fp16, fp8_scales, fp8_buffers = self._build_pipeline_weights()
        max_prompt_len = getattr(self, '_prompt_len', self.max_prompt_len)
        
        # Create FP8 pipeline
        self._pipeline = Pi05PipelineSm89Fp8V3(
            fvk=self._fvk,
            weights_fp8_colmajor=weights_fp8,
    ...
    def _build_pipeline_weights(self):
        ...
        def s_fp8(k):
            t = S8[k]
            # Return scale value (float), not tensor pointer
            # For simple kernel
        ...
        weights_fp8 = {
            # Encoder FP8 weights (ColumnMajor)
        ...
        fp8_scales = {
            "encoder_attn_qkv_w": s_fp8("encoder_attn_qkv_w"),
            "encoder_ffn_gate_w": s_fp8("encoder_ffn_gate_w"),
            "encoder_ffn_up_w": s_fp8("encoder_ffn_up_w"),
            "encoder_ffn_down_w": s_fp8("encoder_ffn_down_w"),
        }
        ...
        weights_fp16 = {
            ...
            "vision_attn_qkv_w": p_fp16("vision_attn_qkv_w"),
            "vision_attn_qkv_b": p_fp16("vision_attn_qkv_b"),
            "vision_attn_o_w": p_fp16("vision_attn_o_w"),
            "vision_attn_o_b": p_fp16("vision_attn_o_b"),
            "vision_ffn_up_w": p_fp16("vision_ffn_up_w"),
            "vision_ffn_up_b": p_fp16("vision_ffn_up_b"),
            "vision_ffn_down_w": p_fp16("vision_ffn_down_w"),
        ...
        fp8_buffers = {
            "encoder_x_fp8": encoder_x_fp8,
            "encoder_x_fp8_ptr": encoder_x_fp8.data_ptr(),
            "encoder_hidden_fp8": encoder_hidden_fp8,
            "encoder_hidden_fp8_ptr": encoder_hidden_fp8.data_ptr(),
        }
        
        return weights_fp8, weights_fp16, fp8_scales, fp8_buffers
